...
export const douyinApi = {
  ...
  // ── 登录相关 ────────────────────────────────────────────
  // 启动扫码登录（打开真实浏览器窗口）
  startLogin: async (): Promise<{ status: string; message: string }> => {
    return api.post('/douyin/login/start', {})
  },
  ...
  getLoginStatus: async (): Promise<{
    ...
    return api.get('/douyin/login/status')
  },

  // 清除已保存的 Cookie
  clearCookies: async (): Promise<{ success: boolean; message: string }> => {
    return api.delete('/douyin/login/cookies')
  },
}
