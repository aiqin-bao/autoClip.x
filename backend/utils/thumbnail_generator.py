        ...
        try:
            ...
            if time_offset == -1.0:
                ...
                if cover_path.exists():
                    ...
                    if verify_result.returncode == 0 and 'width' in verify_result.stdout:
                        use_cover = True
                    else:
                        logger.warning(f"封面图片损坏，跳过: {cover_path}")
                ...
                if use_cover:
                    ...
                    logger.info(f"封面不可用，回退到从视频提取帧: {time_offset}秒")
            else:
                # 使用指定时间点
