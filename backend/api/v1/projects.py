        ...
        if not video_files:
            ...
            if clip and clip.video_path:
                ...
                if video_file_path.exists():
                    ...
                    # video_path 存的是去特殊字符的路径，实际文件名可能含特殊字符
                    # 从路径提取前缀编号（如 "1_xxx.mp4" -> "1"），再 glob 查找实际文件
                    stored_filename = video_file_path.name
                    seq_prefix = stored_filename.split('_')[0] if '_' in stored_filename else None
                    if seq_prefix and seq_prefix.isdigit():
                        matched = list(clips_dir.glob(f"{seq_prefix}_*.mp4"))
                        if matched:
                            video_file = matched[0]
                            # 顺带更新数据库中的路径，保持一致
                            try:
                                clip.video_path = str(matched[0])
                                project_service.db.commit()
                            except Exception:
                                project_service.db.rollback()
                        else:
                            raise HTTPException(status_code=404, detail=f"Clip video file not found: {stored_filename}")
                    else:
                        raise HTTPException(status_code=404, detail=f"Clip video file not found: {clip.video_path}")
            else:
